...
```
python API_Post/manage_password_store.py list                 # nicknames + fields only (never passwords)
python API_Post/manage_password_store.py set sb27             # add/overwrite sb27's web.bd password (prompted)
python API_Post/manage_password_store.py set sb27 --field admin
python API_Post/manage_password_store.py remove sb27 --field admin   # remove one field
python API_Post/manage_password_store.py remove sb27                 # remove the whole robot
```

## First-time setup

1. `python API_Post/manage_password_store.py keygen` -- prints a new keypair.
...
4. `python API_Post/manage_password_store.py set <robot>` for each robot, then commit the updated
   `robot_passwords.age`.
...
1. `python API_Post/manage_password_store.py keygen` -- generate a new keypair.
...
- The GUI only ever reads the store; it never writes it and never persists the key.
